...
class ProdutoRepo:
    ...
    def inserir(cls, produto: Produto) -> Optional[Produto]:
        try:
            with criar_conexao() as conexao:
                cursor = conexao.cursor()
                cursor.execute(SQL_INSERIR_PRODUTO, (produto.nome, produto.preco, produto.descricao, produto.categoria))
    ...
    def alterar(cls, produto: Produto) -> Optional[Produto]:
        try:
            with criar_conexao() as conexao:
                cursor = conexao.cursor()
                cursor.execute(SQL_ALTERAR,(produto.nome, produto.preco, produto.descricao, produto.categoria, produto.id))
            if cursor.rowcount > 0:
                return produto
        except sqlite3.Error as e:
            print(e)
